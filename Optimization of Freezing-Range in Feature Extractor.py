...
# 1) val dataloader
val_dataframe= pd.read_csv('dataframe/val.csv')
...
train_dataframe_newclass= pd.read_csv('dataframe/train.csv')
train_dataframe_repre= pd.read_csv('dataframe/repre.csv') 
...
bn_layer_name_list = []
for name, module in net.feature_extractor.named_modules(): 
    if isinstance(module, nn.BatchNorm2d) and 'downsample' not in name:
        bn_layer_name_list.append(f"{name}.bias")
        

for param in net.parameters():
    param.requires_grad = True
last_frozen_layer_idx = 1
last_frozen_layer_name = bn_layer_name_list[last_frozen_layer_idx]
for name, param in net.feature_extractor.named_parameters():
    param.requires_grad = False
    if name == last_frozen_layer_name:
        break     
print_frozen_layers(net)
    ...
    for i,data in enumerate(train_dataloader): 
        ...
        if ( (step_index-1)// alternation_period< step_index // alternation_period) and (step_index != 0) :
            if Freeze == True:
                for param in net.parameters():
                    param.requires_grad = True
                target_name = bn_layer_name_list[last_frozen_layer_idx]
                for name, param in net.feature_extractor.named_parameters():
                    param.requires_grad = False
                    if name == target_name:
                        break
